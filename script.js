...
function verificarIMC(imc) {
	if (imc < 17) {
		createMessage("Cuidado...esta muy bajo su peso....", "alert")
	} else if (imc > 17 && imc <= 18.49){
		createMessage("Tiene peso bajo...", "warning")
	} else if (imc >= 18.5 && imc <= 24.99){
		createMessage("Peso Normal", "sucess")
	} else if (imc >= 25 && imc <= 29.99){
		createMessage("Excedido de Peso", "warning")
	} else if (imc >= 30 && imc <= 34.99){
		createMessage("Obesidad grado 1", "alert")
	} else {
		createMessage("Obesidad grado 2", "danger")
	}
}
